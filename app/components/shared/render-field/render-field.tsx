...
import { FieldValue, FormField, FormOption } from "./types";
...
export const RenderField: React.FC<RenderFieldProps> = ({
  ...
  if (field.type === "heading") {
    return (
      <h3 key={field.id} className="text-foreground text-lg font-semibold mt-6 mb-2 col-span-1 md:col-span-2 lg:col-span-3">
        {field.label}
      </h3>
    );
  }
  if (field.type === "group" && field.subFields) {
    return (
      <div
        key={field.id}
        className="space-y-4 p-2 rounded-lg bg-base-200"
      >
        {field.label && (
          <label className="block text-foreground text-sm">{field.label}</label>
        )}
        <div className="">
  ...
  if (field.type === "checkboxGroup" && field.subFields) {
    return (
      <div key={field.id} className="space-y-4 rounded-lg bg-base-200">
  ...
  if (field.type === "checkbox") {
    return (
      <div
        key={field.id}
        className="flex items-center p-2  rounded-lg bg-base-200"
      >
        <Checkbox
          id={field.id}
  ...
  if (field.type === "text") {
    return (
      <div
        key={field.id}
        className="flex items-center p-2 rounded-lg bg-base-200 w-full"
      >
        <Input
          id={field.id}
          value={formData[field.id] as string}
