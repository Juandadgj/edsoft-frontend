...
interface QualificationTypeDeleteModalProps {
  open: boolean;
...
export function QualificationTypeDeleteModal({
  open,
  typeQualification,
  revalidatePath = DEFAULT_REVALIDATE_PATH,
  onClose,
}: QualificationTypeDeleteModalProps) {
  const [state, formAction, isPending] = useActionState(deleteQualificationTypeAction, initialState);
  ...
  return (
    <Modal open={open} title="Eliminar tipo de calificación">
      <form action={formAction} className="space-y-4">
        <input type="hidden" name="id_type_qual" value={typeQualification?.id_type_qual ?? ''} />
        ...
        <p className="text-foreground">
          ¿Desea eliminar el tipo de calificación{' '}
          <strong>{typeQualification?.name ?? ''}</strong>? Esta acción no se puede deshacer.
        </p>
